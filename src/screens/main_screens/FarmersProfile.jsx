...
const FarmersProfile = () => {
  ...
  // Mock documents (optional: later replace with Firestore documents)
  ...
  // Verify farmer: update Firestore status
    ...
    <div className="space-y-6 pb-14">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 flex flex-col md:flex-row gap-10">
        <div className="w-full md:w-64 h-64 shrink-0 rounded-3xl overflow-hidden bg-gray-100">
          <img
            src={farmer?.profileImage || "https://images.unsplash.com/photo-1595840245037-33f7c32eb34e?q=80&w=600&auto=format&fit=crop"}
            alt="Farmer Profile"
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-y-8 gap-x-6 mb-6">
              <div>
                ...
                <p className="text-xl font-bold text-gray-900 leading-none">{farmer?.firstName || 'N/A'}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-xl font-bold text-gray-900 leading-none">{farmer?.lastName || 'N/A'}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-base font-bold text-gray-900">{farmer?.phone || 'N/A'}</p>
              </div>
              <div>
                <p className="text-[11px] font-medium text-gray-500 mb-1">Age</p>
                <p className="text-base font-bold text-gray-900">{farmer?.age || 'N/A'}</p>
              </div>
              <div className="md:col-span-3">
                <p className="text-[11px] font-medium text-gray-500 mb-1">Address</p>
                <p className="text-base font-bold text-gray-900">{farmer?.address || 'N/A'}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 flex flex-col md:flex-row gap-10 mt-6">
        <div className="w-full md:w-64 h-64 shrink-0 rounded-3xl overflow-hidden bg-gray-100">
          <img
            src={farmer?.farmImage || "https://images.unsplash.com/photo-1560493676-04071c5f467b?q=80&w=600&auto=format&fit=crop"}
            alt="Farm Map View"
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-6">
            <div className="md:col-span-2">
              ...
              <p className="text-base font-bold text-gray-900">{farmer?.farmLocation || 'N/A'}</p>
            </div>
            <div>
              <p className="text-[11px] font-medium text-gray-500 mb-1">Farm Size</p>
              <p className="text-base font-bold text-gray-900">{farmer?.farmSize || 'N/A'} hectares</p>
            </div>
            <div>
              <p className="text-[11px] font-medium text-gray-500 mb-1">No. of fields</p>
              <p className="text-base font-bold text-gray-900">{farmer?.fields || 'N/A'}</p>
            </div>
            <div className="md:col-span-2">
              <p className="text-[11px] font-medium text-gray-500 mb-1">Crop Type</p>
              <p className="text-base font-bold text-gray-900">{farmer?.cropType || 'N/A'}</p>
            </div>
          </div>
        </div>
      </div>
              ...
              <div key={doc.id} className="border border-gray-200 rounded-xl p-5 flex flex-col gap-4">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => alert(`Viewing ${doc.title}`)} className="w-9 h-9 flex items-center justify-center rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 hover:text-gray-700 transition-colors">
                      <HiOutlineEye className="text-lg" />
                    </button>
                    <button onClick={() => alert(`Downloading ${doc.title}...`)} className="w-9 h-9 flex items-center justify-center rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 hover:text-gray-700 transition-colors">
                      <HiOutlineDownload className="text-lg" />
                    </button>
                    <button onClick={() => handleDeleteDoc(doc.id)} className="w-9 h-9 flex items-center justify-center rounded-lg border border-red-200 text-red-500 hover:bg-red-50 hover:text-red-600 transition-colors">
